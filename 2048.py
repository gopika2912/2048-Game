...
class Game2048(tk.Frame):
        ...
        for i in range(4):
            for j in range(4):
                ...
                if cell_value == 0:
                    ...
                    self.cells[i][j].configure(text=str(cell_value), bg=self.get_color(cell_value), fg=self.get_text_color(cell_value))
        ...
        def merge(row):
            ...
            for i in range(3):
                if row[i] != 0 and row[i] == row[i + 1] and i not in merged:
                    ...
                    row[i + 1] = 0
                    merged.append(i)
            return [num for num in row if num != 0] + [0] * (4 - len([num for num in row if num != 0]))
    ...
    def game_over(self):
        ...
        tk.Label(self, text="Game Over", font=("Verdana", 48, "bold"), bg="#bbada0", fg="white").pack()

    def play_sound(self, sound_type):
        winsound.PlaySound("SystemHand" if sound_type == "merge" else "SystemExclamation", winsound.SND_ALIAS)
